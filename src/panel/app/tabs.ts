...
export type TabId = "settings" | "logs" | "overview";
...
export function createTabs(dom: Dom, tabs: Record<TabId, Tab>) {
  ...
  function setTabUI(next: TabId) {
    ...
    dom.tabOverview.classList.toggle("is-active", is("overview"));
    dom.tabOverview.setAttribute("aria-selected", String(is("overview")));
    dom.viewOverview.hidden = !is("overview");
  ...
  function bind() {
    ...
    dom.tabOverview.addEventListener("click", () => switchTo("overview"));
